...
export function ChatNodePicker({ nodes, value, onChange, loading, error, retry, disabled = false }: {
  ...
  return <div className="min-w-0 space-y-1 text-sm">
    ...
    <select id={id} value={value} onChange={event => onChange(event.target.value)}
      ...
      className="w-full min-w-0 rounded-md border border-subtle bg-raised px-2 py-1.5 text-base text-primary disabled:opacity-60 focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent">
    ...
    <div id={`${id}-status`} className="text-sm text-secondary" role="status">
      {error ? <>{error} <button type="button" className="underline hover:text-primary" onClick={retry}>Retry nodes</button></>
