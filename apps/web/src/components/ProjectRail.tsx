          ...
          <BoundedCollection<ChatSessionSummary | null> key={currentProfile} items={[null, ...liveSessions]} query={query}
            ...
            {(session) => (
              <button key={session?.id ?? 'default'} type="button" onClick={() => onSessionSelect(session?.id ?? null)}
                ...
                {session?.titleSuggestion && <span className="block truncate text-[11px] text-muted">Suggested by {session.titleSuggestion.backendInstance ?? session.titleSuggestion.backend} · {session.titleSuggestion.model}</span>}
              </button>
            )}
          </BoundedCollection>
        </nav>
