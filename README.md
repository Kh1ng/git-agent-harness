...
`alice`). To add or remove trusted authors for one profile, set the allowlist
under its publishing policy:
...
An explicit empty list disables GitHub issue intake for that profile. The same
policy applies to loop discovery and an explicit `gah dispatch --target #123`.
