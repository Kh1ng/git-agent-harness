...
fn dry_run(cfg: &GahConfig, profile: &Profile, args: &DispatchArgs) -> Result<()> {
    ...
    match args.mode.as_str() {
        "improve" | "fix" => {
            let ticket_meta = parse_ticket_metadata(Path::new(&args.target)).ok().flatten();
            let route = dry_run_route(cfg, profile, &args.mode, args, ticket_meta.as_ref());
        ...
        "pm" => {
            if args.target.is_empty() {
                ...
                let route = dry_run_route(cfg, profile, "pm", args, None);
                println!("Backend:      {}", args.backend);
                if let Some(route) = &route {
                    println!("Effective:    {}", route.effective_backend);
                    println!("Routing:      {}", route.routing_reason);
        ...
        "review" => {
            let route = dry_run_route(cfg, profile, "review", args, None);
            println!("Backend:      {}", args.backend);
            if let Some(route) = &route {
                println!("Effective:    {}", route.effective_backend);
                println!("Routing:      {}", route.routing_reason);
...
mod tests {
    ...
    #[test]
    fn build_mr_title_prefers_suggested_title() {
        let ticket = TicketMetadata {
            suggested_mr_title: Some("Fix auth flow".into()),
            ..Default::default()
        };
    ...
    #[test]
    fn build_mr_title_falls_back_to_generic_title() {
        assert_eq!(
            build_mr_title("improve", "repo", false, None),
            "[GAH] Improve: repo"
        );
        assert_eq!(
            build_mr_title("improve", "repo", true, None),
            "[GAH][DRAFT-FAIL] Improve: repo"
        );
    }
}
...
fn dry_run_route(
    ...
    routing::decide(
        ...
        RouteRequest {
            ...
            requested_backend: &args.backend,
            requested_model: args.model.as_deref(),
            recommended_backend: ticket_meta.and_then(|meta| meta.recommended_backend.as_deref()),
            recommended_model: ticket_meta.and_then(|meta| meta.recommended_model.as_deref()),
            session_id: None,
            usage_summary: None,
        },
    )
...
fn parse_ticket_metadata(path: &Path) -> Result<Option<TicketMetadata>> {
    ...
    for line in body.lines().map(str::trim) {
        if let Some(value) = line.strip_prefix("Suggested MR Title:") {
            let value = value.trim();
            if !value.is_empty() {
                meta.suggested_mr_title = Some(value.to_string());
            }
...
fn build_mr_title(
    ...
    if let Some(ticket) = ticket {
        if let Some(title) = ticket.suggested_mr_title.as_deref() {
