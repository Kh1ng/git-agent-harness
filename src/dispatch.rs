...
fn improve(
    ...
    let changed_files = changed_files_against_target(&wt, &profile.default_target_branch);
    let mr_body = build_standard_mr_body(StandardMrBodyContext {
        mode: &args.mode,
        branch: &branch,
...
fn experiment(
    ...
    let changed_files = changed_files_against_target(&wt, &profile.default_target_branch);
    let mr_body = build_experiment_mr_body(ExperimentMrBodyContext {
        branch: &branch,
        target_branch: &profile.default_target_branch,
        route: &route,
        model: &llm.model,
...
mod tests {
    ...
    use super::validate;
    use super::{
        apply_pm_plan, apply_route_to_ledger, build_experiment_mr_body, build_mr_title,
        build_pm_plan_task, build_standard_mr_body, classify_validation_failure_progress,
        collect_pm_preflight, collect_ticket_summaries, first_markdown_heading,
        mark_backend_unavailable_from_output_at, parse_pm_plan, parse_ticket_metadata,
        validation_failure_no_progress_reason, ExperimentMrBodyContext, RouteDecision,
        StandardMrBodyContext, TicketMetadata, ValidationFailureProgress,
    };
    ...
    #[test]
    fn parses_ticket_metadata_for_authoritative_pr_description() {
        let tmp = tempfile::tempdir().unwrap();
        let ticket = tmp
            .path()
            .join("TICKET-094-authoritative-pr-description.md");
        fs::write(
            &ticket,
            "# TICKET-094: Authoritative PR description generation\n\nGoal: Generate PR description from structured work metadata.\n\n## Problem\nCurrent MR body is too sparse.\n\n## Acceptance Criteria\n1. Include sections.\n2. Omit missing sections.\n\n## Affected Files\n- src/dispatch.rs — MR generation\n\n## Constraints\n- Do not include secrets\n\n## Verification Commands\n- `cargo test`\n",
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            Some("Current MR body is too sparse.")
        );
        assert_eq!(
            meta.goal.as_deref(),
            Some("Generate PR description from structured work metadata.")
        );
        assert_eq!(
            meta.acceptance_criteria,
            vec!["Include sections.", "Omit missing sections."]
        );
        assert_eq!(meta.affected_files, vec!["src/dispatch.rs"]);
        assert_eq!(meta.constraints, vec!["Do not include secrets"]);
    ...
    #[test]
    fn rich_mr_body_uses_structured_metadata_and_runtime_results() {
        let ticket = TicketMetadata {
            ticket_id: Some("TICKET-094".into()),
            title: Some("Authoritative PR description generation".into()),
            problem: Some("Current MR body is too sparse.".into()),
            goal: Some("Generate PR description from structured work metadata.".into()),
            acceptance_criteria: vec!["Include sections.".into()],
            constraints: vec!["Do not include secrets".into()],
            verification_commands: vec!["cargo test".into()],
            affected_files: vec!["src/dispatch.rs".into()],
            ..TicketMetadata::default()
        };
        let route = RouteDecision {
            requested_backend: "auto".into(),
        ...
        ledger.insertions = Some(10);
        ledger.deletions = Some(3);
        let validation_commands = vec!["cargo test".into()];
        let changed_files = vec!["src/dispatch.rs".into(), "tests/gah_cli.rs".into()];
        let body = build_standard_mr_body(StandardMrBodyContext {
            mode: "fix",
            branch: "gah/test",
            target_branch: "main",
            route: &route,
    ...
    fn metadata_poor_mr_body_falls_back_to_legacy_template() {
        let ticket = TicketMetadata {
            ticket_id: Some("TICKET-058".into()),
            title: Some("Descriptive Title Here".into()),
            ..TicketMetadata::default()
        };
        let route = RouteDecision {
            requested_backend: "auto".into(),
            effective_backend: "codex".into(),
            requested_model: None,
            effective_model: Some("local/test".into()),
            routing_reason: "explicit".into(),
            fallback_used: false,
            confidence_impact: None,
            human_required: false,
        };
        ...
        assert!(body.contains("Ticket: TICKET-058 Descriptive Title Here"));
        assert!(body.contains("Validation passed: true"));
        assert!(!body.contains("### Work Item"));
    }

    #[test]
    fn experiment_mr_body_reports_changes_and_judge_verdict() {
        let route = RouteDecision {
            requested_backend: "auto".into(),
            effective_backend: "codex".into(),
            requested_model: None,
        ...
        ledger.insertions = Some(4);
        ledger.deletions = Some(1);

        let changed_files = vec!["reports/findings.md".into()];
        let body = build_experiment_mr_body(ExperimentMrBodyContext {
            branch: "gah/exp-test",
            target_branch: "main",
            route: &route,
            model: "gpt-5.4",
        ...
        assert!(body.contains("## GAH Experiment"));
        assert!(body.contains("- `reports/findings.md`"));
        assert!(body.contains("Judge verdict: `ANSWERED`"));
        assert!(body.contains("Actual route: `codex` / `gpt-5.4`"));
    }
...
fn changed_files_against_target(wt: &Path, target_branch: &str) -> Vec<String> {
    git_output(
        &["diff", "--name-only", &format!("{target_branch}...HEAD")],
        wt,
    )
    .map(|output| {
        output
...
fn parse_ticket_metadata(path: &Path) -> Result<Option<TicketMetadata>> {
    ...
    for raw_line in body.lines() {
        ...
        if let Some(value) = line.strip_prefix("Difficulty:") {
            ...
            let value = value.trim();
            if !value.is_empty() {
                meta.problem = Some(value.to_string());
            }
            ...
            let value = trim_after_delimiter(value, &[" — ", " - "]).trim();
            match section {
                Some("Affected Files") => {
                    if !value.is_empty() {
                ...
                Some("Acceptance Criteria") => {
                    if !value.is_empty() {
                        meta.acceptance_criteria.push(value.to_string());
                    }
                }
                Some("Constraints") => {
                    if !value.is_empty() {
                        meta.constraints.push(value.to_string());
                    }
                }
        ...
        } else if let Some(item) = strip_numbered_item(line) {
            match section {
                Some("Acceptance Criteria") => meta.acceptance_criteria.push(item.to_string()),
                Some("Constraints") => meta.constraints.push(item.to_string()),
                _ => {}
            }
        } else if !line.is_empty() {
            match section {
                Some("Problem") if meta.problem.is_none() => {
                    meta.problem = Some(line.to_string());
                }
                Some("Goal") if meta.goal.is_none() => {
                    meta.goal = Some(line.to_string());
                }
                _ => {}
            }
        }
...
fn trim_after_delimiter<'a>(value: &'a str, delimiters: &[&str]) -> &'a str {
    for delimiter in delimiters {
        if let Some((before, _)) = value.split_once(delimiter) {
            return before;
        }
    }
    value
...
fn ticket_supports_authoritative_pr_description(ticket: &TicketMetadata) -> bool {
    ticket.problem.is_some()
        || ticket.goal.is_some()
        || !ticket.acceptance_criteria.is_empty()
        || !ticket.constraints.is_empty()
        || !ticket.verification_commands.is_empty()
        || !ticket.affected_files.is_empty()
}

struct StandardMrBodyContext<'a> {
    mode: &'a str,
    branch: &'a str,
    target_branch: &'a str,
    ...
    ticket: Option<&'a TicketMetadata>,
    validation_commands: &'a [String],
    ledger: &'a LedgerEntry,
    changed_files: &'a [String],
}

fn build_standard_mr_body(ctx: StandardMrBodyContext<'_>) -> String {
    let Some(ticket) = ctx
        .ticket
        .filter(|ticket| ticket_supports_authoritative_pr_description(ticket))
    else {
        return format!(
            "## GAH {} mode\n\nTicket: {}\nBackend/model: `{}` / `{}`\nBranch: `{}`\nTarget: `{}`\nValidation passed: {}\n\nGenerated by `gah dispatch`.",
            ctx.mode,
            render_ticket_label(ctx.ticket),
            ctx.route.effective_backend,
            ctx.model,
            ctx.branch,
            ctx.target_branch,
    ...
    let mut sections = vec![format!("## GAH {} mode", ctx.mode)];

    let mut work_item = Vec::new();
    if let Some(ticket_id) = ticket.ticket_id.as_deref() {
        work_item.push(format!("ID: `{ticket_id}`"));
    }
    if let Some(title) = ticket.title.as_deref() {
        work_item.push(format!("Title: {title}"));
    }
    ...
    if !ctx.changed_files.is_empty() {
        changes.push("Files changed:".to_string());
        changes.extend(ctx.changed_files.iter().map(|file| format!("- `{file}`")));
    } else if !ticket.affected_files.is_empty() {
        changes.push("Files changed:".to_string());
        changes.extend(
            ticket
                .affected_files
    ...
    if let Some(files_changed) = ctx.ledger.files_changed {
        changes.push(format!(
            "Diff summary: {} files, +{}, -{}",
            files_changed,
            ctx.ledger.insertions.unwrap_or(0),
            ctx.ledger.deletions.unwrap_or(0)
        ));
    }
    if !changes.is_empty() {
    ...
    let commands = if ctx.validation_commands.is_empty() {
        &ticket.verification_commands
    } else {
        ctx.validation_commands
    };
    if !commands.is_empty() {
        validation.push("Commands:".to_string());
        validation.extend(commands.iter().map(|cmd| format!("- `{cmd}`")));
    }
    if let Some(result) = ctx.ledger.validation_result.as_deref() {
        validation.push(format!("Outcome: `{result}`"));
    } else {
    ...
    sections.push(format!(
        "### Backend/Model\nActual route: `{}` / `{}`",
        ctx.route.effective_backend, ctx.model
    ));
    sections.push(format!(
        "### Attempts\nStarted/completed: `{}` / `{}`\nFallback used: `{}`",
        ctx.ledger.attempts_started, ctx.ledger.attempts_completed, ctx.route.fallback_used
    ));

    let mut failure = Vec::new();
    if ctx.validation_failed {
        failure.push("Final state: validation-failing draft".to_string());
    }
    if let Some(class) = ctx.ledger.failure_class.as_deref() {
        failure.push(format!("Failure class: `{class}`"));
    }
    if let Some(stage) = ctx.ledger.failure_stage.as_deref() {
        failure.push(format!("Failure stage: `{stage}`"));
    }
    if let Some(summary) = ctx.ledger.error_summary.as_deref() {
        failure.push(format!("Summary: {summary}"));
    }
    if !failure.is_empty() {
    ...
    sections.push("Generated by `gah dispatch`.".to_string());
    sections.join("\n\n")
}

struct ExperimentMrBodyContext<'a> {
    branch: &'a str,
    target_branch: &'a str,
    route: &'a RouteDecision,
    model: &'a str,
...
fn build_experiment_mr_body(ctx: ExperimentMrBodyContext<'_>) -> String {
    let mut sections = vec!["## GAH Experiment".to_string()];
    ...
    sections.push("Generated by `gah dispatch --mode experiment`.".to_string());
    sections.join("\n\n")
}
