...
fn improve(
    ...
    let mr_body = build_mr_description(
        &args.mode,
        ticket_meta.as_ref(),
        &route.effective_backend,
        &llm.model,
        ...
        ledger.attempts_started,
        ledger.attempts_completed,
...
fn experiment(
    ...
    let mr_body = build_mr_description(
        ...
        &files_changed,
        Some(artifact_count),
        Some(if answered { "ANSWERED" } else { "PARTIAL" }),
    );
    ledger.mr_attempted = true;
...
mod tests {
    ...
    use super::validate;
    use super::{
        apply_pm_plan, apply_route_to_ledger, build_mr_description, build_mr_title,
        build_pm_plan_task, classify_validation_failure_progress, collect_pm_preflight,
        collect_ticket_summaries, first_markdown_heading, mark_backend_unavailable_from_output_at,
        parse_pm_plan, parse_ticket_metadata, validation_failure_no_progress_reason, RouteDecision,
    ...
    #[test]
    fn test_build_mr_description_metadata_poor_fallback() {
        ...
        assert!(desc.contains("## GAH fix mode"));
        assert!(desc.contains("Ticket: n/a"));
        assert!(desc.contains("Backend/model: `codex` / `gpt-5.4`"));
        assert!(desc.contains("Branch: `gah/gah-12345`"));
        assert!(desc.contains("Target: `main`"));
        assert!(desc.contains("Validation passed: true"));
        assert!(desc.contains("Generated by `gah dispatch`."));
    }

    #[test]
    fn test_build_mr_description_metadata_rich() {
        let ticket = TicketMetadata {
            ticket_id: Some("TICKET-123".to_string()),
            title: Some("Implement feature X".to_string()),
            goal: Some("The goal is feature X".to_string()),
            problem: Some("Feature X is missing".to_string()),
            ..TicketMetadata::default()
        };
        let desc = build_mr_description(
            "improve",
        ...
        assert!(desc.contains("## GAH improve mode"));
        assert!(desc.contains("### Work Item\n\nTicket: TICKET-123 Implement feature X"));
        assert!(desc.contains("### Problem\n\nFeature X is missing"));
        assert!(desc.contains("### Goal\n\nThe goal is feature X"));
        assert!(desc.contains("### Changes\n\n- `src/main.rs`"));
        assert!(
            desc.contains("### Validation\n\n**Commands**:\n- `cargo test`\n\n**Outcome**: failed")
        );
        assert!(desc.contains("### Backend/model used\n\nBackend/model: `claude` / `sonnet-3.5`"));
        assert!(desc.contains(
            "### Attempts\n\n- **Started**: 2\n- **Completed**: 1\n- **Fallback used**: true"
        ));
        assert!(desc.contains("### Failure/stop state\n\nValidation failed"));
        assert!(desc.contains("### Branch\n\n- **Source branch**: `gah/gah-67890`"));
        assert!(desc.contains("Generated by `gah dispatch`."));
    }

    #[test]
    fn test_build_mr_description_experiment() {
        // Poor metadata fallback in experiment mode
        let desc_poor = build_mr_description(
            "experiment",
            None,
            "agy",
            "gpt-4",
        ...
        assert!(desc_poor.contains("## GAH Experiment"));
        assert!(desc_poor.contains("Backend/model: `agy` / `gpt-4`"));
        assert!(desc_poor.contains("Branch: `gah/exp-123`"));
        assert!(desc_poor.contains("Target: `main`"));
        assert!(desc_poor.contains("Artifacts: 3"));
        assert!(desc_poor.contains("Judge verdict: ANSWERED"));
        assert!(desc_poor.contains("Generated by `gah dispatch --mode experiment`."));

        // Rich metadata in experiment mode
        let ticket = TicketMetadata {
            ticket_id: Some("TICKET-999".to_string()),
            title: Some("Run experiment Z".to_string()),
            goal: Some("To verify Z".to_string()),
            problem: Some("Z is unverified".to_string()),
            ..TicketMetadata::default()
        };
        let desc_rich = build_mr_description(
            "experiment",
            Some(&ticket),
            "agy",
            "gpt-4",
        ...
        assert!(desc_rich.contains("## GAH Experiment"));
        assert!(desc_rich.contains("### Work Item\n\nTicket: TICKET-999 Run experiment Z"));
        assert!(desc_rich.contains("### Problem\n\nZ is unverified"));
        assert!(desc_rich.contains("### Goal\n\nTo verify Z"));
        assert!(desc_rich.contains("### Changes\n\n- `src/main.rs`"));
        assert!(desc_rich.contains("### Backend/model used\n\nBackend/model: `agy` / `gpt-4`"));
        assert!(desc_rich.contains(
            "### Attempts\n\n- **Started**: 1\n- **Completed**: 1\n- **Fallback used**: false"
        ));
        assert!(desc_rich.contains("### Failure/stop state\n\nJudge verdict: PARTIAL"));
        assert!(desc_rich.contains("### Branch\n\n- **Source branch**: `gah/exp-123`"));
        assert!(desc_rich.contains("Generated by `gah dispatch --mode experiment`."));
    }
}
...
fn parse_ticket_metadata(path: &Path) -> Result<Option<TicketMetadata>> {
    ...
    if !problem_lines.is_empty() {
        let joined = problem_lines.join("\n").trim().to_string();
        if !joined.is_empty() {
            meta.problem = Some(joined);
        }
    }
    Ok(Some(meta))
}
...
fn build_mr_description(
    ...
    experiment_artifacts: Option<usize>,
    experiment_verdict: Option<&str>,
) -> String {
    let Some(ticket) = ticket_meta else {
        // Fall back to existing template
        if mode == "experiment" {
            let artifact_count = experiment_artifacts.unwrap_or(0);
            let answered = experiment_verdict.unwrap_or("PARTIAL") == "ANSWERED";
            return format!(
                "## GAH Experiment\n\nBackend/model: `{}` / `{}`\nBranch: `{}`\nTarget: `{}`\n\
                 Artifacts: {}\nJudge verdict: {}\n\n\
    ...
    // Metadata-rich layout
    let header = if mode == "experiment" {
        "## GAH Experiment".to_string()
    } else {
        format!("## GAH {} mode", mode)
    };

    let work_item = match (ticket.ticket_id.as_deref(), ticket.title.as_deref()) {
        (Some(id), Some(title)) => Some(format!("### Work Item\n\nTicket: {} {}", id, title)),
        (Some(id), None) => Some(format!("### Work Item\n\nTicket: {}", id)),
        (None, Some(title)) => Some(format!("### Work Item\n\nTicket: {}", title)),
        (None, None) => None,
    };

    let problem_sec = ticket
        .problem
        .as_ref()
        .filter(|s| !s.is_empty())
        .map(|p| format!("### Problem\n\n{}", p));
    ...
    let changes_sec = if !files_changed.is_empty() {
        let files_list = files_changed
            .iter()
            .map(|f| format!("- `{}`", f))
            .collect::<Vec<_>>()
            .join("\n");
    ...
    let validation_sec = if mode != "experiment" && !validation_commands.is_empty() {
        let cmd_list = validation_commands
            .iter()
            .map(|c| format!("- `{}`", c))
            .collect::<Vec<_>>()
            .join("\n");
        let outcome = if validation_failed {
            "failed"
        } else {
            "passed"
        };
    ...
    let backend_model_sec = Some(format!(
        "### Backend/model used\n\nBackend/model: `{}` / `{}`",
        backend, model
    ));

    let attempts_sec = Some(format!(
        "### Attempts\n\n- **Started**: {}\n- **Completed**: {}\n- **Fallback used**: {}",
        attempts_started, attempts_completed, fallback_used
    ));

    let failure_sec = if mode != "experiment" && validation_failed {
        Some("### Failure/stop state\n\nValidation failed".to_string())
    } else if mode == "experiment" {
        if let Some(verdict) = experiment_verdict {
            if verdict == "PARTIAL" || verdict == "UNANSWERED" {
                Some(format!(
                    "### Failure/stop state\n\nJudge verdict: {}",
                    verdict
                ))
    ...
    let branch_sec = Some(format!("### Branch\n\n- **Source branch**: `{}`", branch));

    let footer = if mode == "experiment" {
        "Generated by `gah dispatch --mode experiment`."
    } else {
        "Generated by `gah dispatch`."
    };
    ...
    if let Some(sec) = work_item {
        sections.push(sec);
    }
    if let Some(sec) = problem_sec {
        sections.push(sec);
    }
    if let Some(sec) = goal_sec {
        sections.push(sec);
    }
    if let Some(sec) = changes_sec {
        sections.push(sec);
    }
    if let Some(sec) = validation_sec {
        sections.push(sec);
    }
    if let Some(sec) = backend_model_sec {
        sections.push(sec);
    }
    if let Some(sec) = attempts_sec {
        sections.push(sec);
    }
    if let Some(sec) = failure_sec {
        sections.push(sec);
    }
    if let Some(sec) = branch_sec {
        sections.push(sec);
    }
