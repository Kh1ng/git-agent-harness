...
impl ExecutionIdentity {
    ...
    pub fn validate_for_persistence(&self) -> anyhow::Result<()> {
        validate_operator_label("runner kind", &self.runner_kind)?;
        validate_operator_label("requested backend", &self.requested_backend)?;
        validate_operator_label("logical backend", &self.logical_backend)?;
        validate_operator_label("backend instance", &self.backend_instance)?;
        if let Some(label) = self.account_label.as_deref() {
            validate_operator_label("account label", label)?;
        }
        if let Some(label) = self.auth_source_label.as_deref() {
            validate_operator_label("auth source label", label)?;
        }
        if let Some(label) = self.quota_pool.as_deref() {
            validate_operator_label("quota pool", label)?;
        }
        Ok(())
    }
}
