...
/// every ledger write funnels through `append` (~50 call sites across the
/// crate), so this is the one place that can catch every genuine failure
/// without scattering a notify call across each site individually. Fires
/// per-attempt on a genuine failure (`agent_no_progress`/`agent_failure`/
/// `context_limit_exceeded`, the same classifier the controller already
/// uses to decide retry/escalate), not just once retries are fully
/// exhausted -- deliberately more eager than the "retries exhausted" doc
/// comment on the enum variant, since the operator asked to hear about a
/// real bug as it happens, not only once the harness gives up on it.
/// Routine backpressure (quota exhaustion, node admission deferred, no
/// eligible backend) is `harness_error`/`backend_error`, not covered by
/// `is_genuine_agent_failure`, so it stays silent here as before.
fn notify_if_genuine_failure(cfg: &GahConfig, entry: &LedgerEntry) {
...
mod tests {
    ...
    #[test]
    fn append_fires_dispatch_failed_notification_only_for_genuine_failures() {
        ...
        let mut entry2 = super::super::LedgerEntry::new(
            "test",
            &ledger_tests::profile(),
            "vibe",
            "improve",
