...
impl TelemetryExporter {
        ...
        F: FnMut(&ExportedTelemetryRecord) -> Result<()>
    {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            let record: ExportedTelemetryRecord = serde_json::from_str(line)
                .with_context(|| format!("Parsing JSONL record at line {} in {}", line_num + 1, path.display()))?;
    ...
    pub fn export_from_entries(&mut self, entries: &[LedgerEntry]) -> Result<()> {
        ...
        log::info!("Exporting {} new telemetry records", records_to_export.len());
    ...
    pub fn export_records(&self, records: &[ExportedTelemetryRecord]) -> Result<()> {
        ...
        let mut attempt_usage_by_partition: BTreeMap<PartitionKey, Vec<&ExportedTelemetryRecord>> = BTreeMap::new();
        let mut quota_obs_by_partition: BTreeMap<PartitionKey, Vec<&ExportedTelemetryRecord>> = BTreeMap::new();
        let mut task_outcome_by_partition: BTreeMap<PartitionKey, Vec<&ExportedTelemetryRecord>> = BTreeMap::new();
        let mut review_outcome_by_partition: BTreeMap<PartitionKey, Vec<&ExportedTelemetryRecord>> = BTreeMap::new();
        ...
        for record in records {
            ...
            match record {
                ExportedTelemetryRecord::AttemptUsage(_) => {
                    attempt_usage_by_partition.entry(partition_key).or_default().push(record);
                }
                ExportedTelemetryRecord::QuotaObservation(_) => {
                    quota_obs_by_partition.entry(partition_key).or_default().push(record);
                }
                ExportedTelemetryRecord::TaskOutcome(_) => {
                    task_outcome_by_partition.entry(partition_key).or_default().push(record);
                }
                ExportedTelemetryRecord::ReviewOutcome(_) => {
                    review_outcome_by_partition.entry(partition_key).or_default().push(record);
                }
            }
        }
    ...
    fn ensure_repository_structure(&self) -> Result<()> {
        ...
        if !raw_dir.exists() {
            fs::create_dir_all(&raw_dir)
                .with_context(|| format!("Creating telemetry raw directory: {}", raw_dir.display()))?;
        ...
        for subdir in ["usage", "quota", "outcomes"] {
            ...
            if !path.exists() {
                fs::create_dir_all(&path)
                    .with_context(|| format!("Creating telemetry {} directory: {}", subdir, path.display()))?;
        ...
        if !exports_dir.exists() {
            fs::create_dir_all(&exports_dir)
                .with_context(|| format!("Creating telemetry exports directory: {}", exports_dir.display()))?;
            log::info!("Created telemetry exports directory: {}", exports_dir.display());
        }
        ...
        if !manifests_dir.exists() {
            fs::create_dir_all(&manifests_dir)
                .with_context(|| format!("Creating telemetry manifests directory: {}", manifests_dir.display()))?;
            log::info!("Created telemetry manifests directory: {}", manifests_dir.display());
        }
        ...
        if !schemas_dir.exists() {
            fs::create_dir_all(&schemas_dir)
                .with_context(|| format!("Creating telemetry schemas directory: {}", schemas_dir.display()))?;
            log::info!("Created telemetry schemas directory: {}", schemas_dir.display());
        }
    ...
    fn export_to_directory(
        ...
        for (partition_key, records) in records_by_partition {
            ...
            let mut file = OpenOptions::new()
                ...
                .append(true)
                .open(&file_path)
                .with_context(|| format!("Opening telemetry file for append: {}", file_path.display()))?;
            
            for record in records {
                let json = serde_json::to_string(record)
                    .with_context(|| format!("Serializing telemetry record"))?;
                writeln!(file, "{}", json)
                    .with_context(|| format!("Writing telemetry record to {}", file_path.display()))?;
            }
            
            // Sync to ensure data is written
            file.sync_all()?;
            
            log::info!("Exported {} records to {}", records.len(), file_path.display());
        }
    ...
    fn generate_manifests(
        ...
        for partition_key in all_partitions {
            let manifest = self.generate_partition_manifest(
                partition_key,
                attempt_usage_by_partition.get(&partition_key).map(|v| v.len()).unwrap_or(0),
                quota_obs_by_partition.get(&partition_key).map(|v| v.len()).unwrap_or(0),
                task_outcome_by_partition.get(&partition_key).map(|v| v.len()).unwrap_or(0),
                review_outcome_by_partition.get(&partition_key).map(|v| v.len()).unwrap_or(0),
            )?;
            
            let manifest_dir = self.config.telemetry_repo_path.join("manifests");
            let manifest_path = manifest_dir.join(format!("{}.json", partition_key.to_date_string()));
    ...
    fn generate_partition_manifest(
        ...
        let total_records = attempt_usage_count + quota_obs_count + task_outcome_count + review_outcome_count;
