use super::support::{isolate_command, test_tempdir, FakeBackend, IsolatedCommand};
...
pub(crate) fn spawn_bin() -> IsolatedCommand<ProcessCommand> {
...
pub(crate) fn make_fake_bin(dir: &std::path::Path, name: &str) {
...
pub(crate) fn make_fake_bin_with_body(dir: &std::path::Path, name: &str, body: &str) {
    let path = dir.join(name);
...
pub(crate) fn make_fake_github_review_api(dir: &std::path::Path) {
...
pub(crate) fn prepend_path(dir: &std::path::Path) -> String {
...
pub(crate) fn add_origin_and_feature_commit(repo: &std::path::Path) {
...
pub(crate) fn checkout_branch(repo: &std::path::Path, branch: &str) {
    ProcessCommand::new("git")
...
pub(crate) fn configure_git_url_instead_of(home: &std::path::Path, from: &str, to: &str) {
    fs::write(
        home.join(".gitconfig"),
        format!("[url \"{}\"]\n\tinsteadOf = {}\n", to, from),
    )
