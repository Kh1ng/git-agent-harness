...
| --- | --- | --- |
| Choose a project or an existing chat | [Projects](../apps/web/src/pages/ProjectsPage.tsx) lists every project, its open and archived chats, and the issues and pull requests still waiting for one. Each issue or pull request starts a seeded chat in one step. [ProjectRail](../apps/web/src/components/ProjectRail.tsx) groups projects by owner node and imports new repositories. | Remote projects use blank chats: issue and pull-request setup runs on the central node. |
...
| Use project chat | [Chat](../apps/web/src/pages/ManagerChatPage.tsx) opens a blank conversation: the first message names it and creates its session. It provides streaming replies/tools, permissions, stop/steer, models, and node selection. The composer's [provider picker](../apps/web/src/components/ProviderPicker.tsx) shows favorites and recent picks, with the full catalog in a searchable [model browser](../apps/web/src/components/ModelBrowser.tsx). An authenticated Telegram bridge maps paired identities to exact profile scopes and one-time action cards. | Remote issue/PR-seeded sessions and remote previews remain unsupported. Telegram rejects broad approvals, non-text attachments, and remote slash commands. |
