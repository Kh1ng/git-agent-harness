...
| --- | --- | --- |
| Tauri desktop | A local connection window saves the central address. A separate webview opens the shared dashboard. The app checks tools and starts/stops an optional worker. | [Native commands](../apps/desktop/main.rs), [connection UI](../apps/desktop/src/main.ts), [Windows setup](WINDOWS_NODE_SETUP.md). The local command window rejects remote navigation. Windows uses WSL for the worker. |
| macOS presence | Dock, tray, and launch-window preferences persist. Defaults show the tray and a window. The app preserves a way to reopen its controls. | `Presence`, `save_presence`, and the combination/serialization test in [main.rs](../apps/desktop/main.rs). Packaged Dock/tray transitions still need manual verification. |
| Browser dashboard | React provides Overview, Work, Telemetry, Quota, Events, Settings, Chat, Git, and Nodes. The same app runs in the desktop dashboard webview. | [App routing](../apps/web/src/App.tsx), [navigation tests](../apps/web/tests/component/Navigation.spec.tsx), [browser smoke tests](../apps/web/tests/e2e/smoke.spec.ts). Navigation uses React state rather than URL routes. |
...
| iOS/Android | The shared dashboard and QR exchange provide browser control. The intended native apps are control-only clients. | [Mobile scoping](../apps/desktop/MOBILE_SCOPING.md) records packaging work. Native installation, scanners, credential persistence, and background/resume remain unverified. Icons alone do not establish native support. |
...
| Read activity | [Events](../apps/web/src/pages/EventsPage.tsx) filters controller events and refreshes every 30 seconds, on demand, and after reconnect. | This is not a durable event replay protocol or a native notification center. #941 remains open. |
...
| Use project chat | [Chat](../apps/web/src/pages/ManagerChatPage.tsx) provides sessions, streaming replies/tools, permissions, stop/steer, models, and node selection. [ProjectRail](../apps/web/src/components/ProjectRail.tsx) groups projects by owner node. | Remote issue/PR-seeded sessions and remote previews remain unsupported. Adapter capabilities differ, and reconnect does not establish durable command replay. |
...
| Phone control | Complete authenticated reconnect/replay, PWA behavior, native packaging, and notifications. Test device-specific interactions and background/resume. | [#529](https://github.com/Kh1ng/git-agent-harness/issues/529), [#534](https://github.com/Kh1ng/git-agent-harness/issues/534), [#526](https://github.com/Kh1ng/git-agent-harness/issues/526), [#936](https://github.com/Kh1ng/git-agent-harness/issues/936), [#941](https://github.com/Kh1ng/git-agent-harness/issues/941), [#798](https://github.com/Kh1ng/git-agent-harness/issues/798) |
